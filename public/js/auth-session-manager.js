const AuthSessionManager = {
  ...
  async initSessionCheck() {
    ...
    try {
      ...
      error.textContent = 'Could not check your session. Check your connection and reload.';
  ...
  resetSession() {},
  getSessionStatus() { return { isChecked: true, isChecking: false }; }
};
...
function installDesktopSidebarCollapse() {
  ...
  style.textContent = `
    @media (min-width:1024px){
      .admin-sidebar,.admin-main,.sidebar-collapse-btn{transition:width .25s ease,margin-left .25s ease,max-width .25s ease,left .25s ease}
      .admin-sidebar{overflow-x:hidden}
      .sidebar-collapse-btn{display:inline-flex;align-items:center;justify-content:center;position:fixed;top:28px;left:280px;transform:translateX(-50%);width:32px;height:32px;padding:0;border:1px solid rgba(255,255,255,.16);border-radius:999px;background:var(--charcoal);color:rgba(255,255,255,.82);box-shadow:0 4px 14px rgba(28,26,23,.2);cursor:pointer;z-index:1960}
      .sidebar-collapse-btn:hover,.sidebar-collapse-btn:focus-visible{color:#fff;background:var(--earth-dark);border-color:var(--earth-light)}
      .sidebar-collapse-btn svg{width:16px;height:16px;fill:none;stroke:currentColor;stroke-width:2;transition:transform .25s ease}
      body.admin-sidebar-collapsed .admin-sidebar{width:84px}
      body.admin-sidebar-collapsed .admin-main{margin-left:84px;max-width:calc(100vw - 84px)}
      body.admin-sidebar-collapsed .sidebar-collapse-btn{left:84px}
      body.admin-sidebar-collapsed .sidebar-collapse-btn svg{transform:rotate(180deg)}
      body.admin-sidebar-collapsed .admin-sidebar .logo{font-size:0;padding:0;text-align:center;margin-bottom:32px}
      body.admin-sidebar-collapsed .admin-sidebar .logo::after{content:'NH';display:inline-block;font-family:var(--font-display);font-size:1.2rem;color:#fff;letter-spacing:.04em}
      body.admin-sidebar-collapsed .admin-sidebar .nav-section-title,body.admin-sidebar-collapsed .admin-sidebar .nav-link span{display:none}
      body.admin-sidebar-collapsed .admin-sidebar .nav-section{margin-bottom:18px}
      body.admin-sidebar-collapsed .admin-sidebar .nav-link{justify-content:center;gap:0;padding:13px 0;border-left:0;border-right:3px solid transparent}
      body.admin-sidebar-collapsed .admin-sidebar .nav-link:hover{transform:none;border-left-color:transparent;border-right-color:rgba(196,168,130,.55)}
      body.admin-sidebar-collapsed .admin-sidebar .nav-link.active{border-left-color:transparent;border-right-color:var(--earth-light)}
      body.admin-sidebar-collapsed .admin-sidebar .nav-icon{width:22px;height:22px}
    }
  ...
  button.addEventListener('click', () => apply(!document.body.classList.contains('admin-sidebar-collapsed')));
...
function installAdminInteractionGuard() {
  ...
  adminPage.addEventListener('click', event => {
    ...
    if (inlineAction.includes('logout()') && typeof window.logout === 'function') {
      event.preventDefault();
      event.stopImmediatePropagation();
      Promise.resolve(window.logout()).finally(() => {
        if (navItem.closest('#adminDrawer') && typeof window.closeDrawer === 'function') window.closeDrawer();
      });
      return;
    }
...
window.addEventListener('sessionExpired', () => {
  ...
  const error = document.getElementById('loginError');
  error.textContent = 'Your session has expired. Sign in to continue.';
  error.classList.add('show');
});
