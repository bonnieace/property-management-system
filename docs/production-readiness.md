...
2. On the restored database with migrations 001–017 installed, run `npm run upgrade:preflight`. It performs a read-only, consistent-snapshot audit and returns counts only, without tenant details. Resolve blockers for mismatched property/tenant/unit relationships, duplicate rent periods and case-insensitive duplicate accounts; review orphan-tenant and pending-payment warnings. The command exits nonzero on blockers and never repairs data automatically. Rehearse all migrations against this restored copy. Resolve case-insensitive duplicate usernames/emails before migration 019. Check that contract/deposit property IDs exist and match their units. Review tenants with no contracts/deposits: migration cannot infer their property; they remain visible only to a platform administrator until assigned by a reviewed database maintenance operation.
...
## Synthetic upgrade rehearsal

`test/upgrade.test.js` builds the original 001–017 schema, inserts a tenant shared by two properties with leases, partial rent payments and confirmed deposits, plus an unassigned tenant, then upgrades through 019. It verifies tenant splitting, all relationship links, unchanged financial balances, preserved published content, unknown historical deposit breakdowns and repeat migration behaviour. It also verifies preflight detection of a mismatched payment. This fixture rehearsal uses disposable PGlite; a restore of the actual installation still needs to be rehearsed before deployment.
